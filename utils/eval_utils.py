...
def eval_rendering(
    ...
    render_save_dir = os.path.join(save_dir, "render", str(iteration))
    mkdir_p(render_save_dir)
    image_save_dir = os.path.join(save_dir, "render/images")
    mkdir_p(image_save_dir)

    frame_results = []

    frame_indices = [idx for idx in range(0, end_idx, interval) if idx not in kf_indices]
