...
class GaussianModel:
    ...
    def create_pcd_from_image(self,
                              cam_info,
                              init=False,
                              scale=2.0,
                              depthmap=None):
        cam = cam_info
        image_ab = (torch.exp(
            cam.exposure_a)) * cam.original_image + cam.exposure_b
        image_ab = torch.clamp(image_ab, 0.0, 1.0)
        rgb_raw = (image_ab * 255).byte().permute(
        ...
        if depthmap is not None:
            ...
            if self.config["Dataset"]["sensor_type"] == "monocular":
                depth_raw = (
                    ...
                    (np.random.randn(depth_raw.shape[0], depth_raw.shape[1]) -
                     0.5) * 0.05) * scale
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        pcd = BasicPointCloud(points=new_xyz,
                              colors=new_rgb,
                              normals=np.zeros((new_xyz.shape[0], 3)))
        self.ply_input = pcd

        fused_point_cloud = torch.from_numpy(np.asarray(
            pcd.points)).float().cuda()
        fused_color = RGB2SH(
            torch.from_numpy(np.asarray(pcd.colors)).float().cuda())
        ...
        rots[:, 0] = 1
        opacities = inverse_sigmoid(0.5 * torch.ones(
    ...
    def extend_from_pcd(self, fused_point_cloud, features, scales, rots,
                        opacities, kf_id):
    ...
    def extend_from_pcd_seq(self,
        ...
        fused_point_cloud, features, scales, rots, opacities = (
            self.create_pcd_from_image(cam_info,
                                       init,
                                       scale=scale,
                                       depthmap=depthmap))
        self.extend_from_pcd(fused_point_cloud, features, scales, rots,
                             opacities, kf_id)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        selected_pts_mask = torch.where(padded_grad >= grad_threshold, True,
                                        False)
        ...
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(
            -1) + self.get_xyz[selected_pts_mask].repeat(N, 1)
        ...
        prune_filter = torch.cat((
            selected_pts_mask,
            torch.zeros(N * selected_pts_mask.sum(), device="cuda",
                        dtype=bool),
        ))
